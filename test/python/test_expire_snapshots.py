...
def registered_metadata_table(rest_catalog, seeded_table):
    ...
    def register(source_identifier, target_identifier, snapshot_count, mutate_metadata):
        source_table, _ = seeded_table(source_identifier, snapshot_count, distinct_timestamps=True)
...
class TestExpireSnapshots:
    ...
    def test_negative_timestamp_cutoff_uses_mathematical_ceil(self, seeded_table, duckdb_runner):
        table, _ = seeded_table("default.expire_snapshots_negative_cutoff_source", 2)
        ...
        snapshots[0]["timestamp-ms"] = -2
        snapshots[1]["timestamp-ms"] = -1

        with _serve_table_metadata(table.metadata_location, metadata) as port:
            with duckdb_runner() as test:
                _attach_metadata_catalog(test, "negative_cutoff_datalake", port)
                test.statement_ok("BEGIN")
                _check_expiration(
                    test,
                    "negative_cutoff_datalake.default.negative_cutoff",
                    (1, 1),
                    "retain_last => 10",
                    "older_than => make_timestamp(-1001)",
                )
                test.query(
                    "I",
                    "SELECT count(*) FROM iceberg_snapshots(negative_cutoff_datalake.default.negative_cutoff)",
                    [(1,)],
                )
                test.statement_ok("ROLLBACK")
    ...
    def test_non_main_reference_is_rejected(
        self,
        ref_type,
        registered_metadata_table,
        duckdb_runner,
    ):
        def replace_main_with_non_main(metadata):
            current_id = metadata["current-snapshot-id"]
            metadata["refs"] = {
                "release": {"snapshot-id": current_id, "type": ref_type},
            }

        _, table_name = registered_metadata_table(
            f"default.expire_snapshots_{ref_type}_source",
            f"default.expire_snapshots_{ref_type}_rejected",
            2,
            replace_main_with_non_main,
        )

        with duckdb_runner() as test:
            test.statement_error(
                _expire_query(table_name),
                'found unsupported snapshot reference "release"',
            )
            test.query("I", f"SELECT count(*) FROM iceberg_snapshots({table_name})", [(2,)])

    @JAVA_FIXTURE_ONLY
    def test_main_plus_non_main_refs_report_first_name_without_staging(
        self,
        registered_metadata_table,
        duckdb_runner,
    ):
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            ("main-max-ref-age-zero", "Invalid max-ref-age-ms"),
    ...
    def test_invalid_explicit_main_metadata(
        ...
        def make_main_invalid(metadata):
            ...
            elif shape == "main-max-ref-age-zero":
                metadata["refs"]["main"]["max-ref-age-ms"] = 0
    ...
    def test_null_refs_uses_implicit_main(self, seeded_table, duckdb_runner):
        table, _ = seeded_table("default.expire_snapshots_null_refs_source", 2)
        metadata = table.metadata.model_dump(mode="json", by_alias=True, exclude_none=True)
        metadata["refs"] = None
        current_id = metadata["current-snapshot-id"]
        ...
        with _serve_table_metadata(table.metadata_location, metadata) as port:
            with duckdb_runner() as test:
                _attach_metadata_catalog(test, "null_refs_datalake", port)
                _check_expiration(
                    test,
                    "null_refs_datalake.default.null_refs",
                    (0, 2),
                    "retain_last => 10",
    ...
    @JAVA_FIXTURE_ONLY
    @pytest.mark.parametrize(
        ("case", "timestamp_ms", "expected"),
        (
            ("old", 100, (1, 0)),
            ("recent", 200, (0, 1)),
        ),
    )
    def test_no_current_single_snapshot_uses_global_cutoff(
        self,
        case,
        timestamp_ms,
        expected,
        ...
        def remove_main(metadata):
            ...
            metadata.pop("refs", None)
            metadata["snapshot-log"] = []
            metadata["snapshots"][0]["timestamp-ms"] = timestamp_ms

        table, table_name = registered_metadata_table(
            f"default.expire_snapshots_no_current_{case}_source",
            f"default.expire_snapshots_no_current_{case}",
            1,
            remove_main,
        )
        ...
        table.refresh()
        expected_ids = set() if timestamp_ms < 150 else {snapshot_id}
        assert {snapshot.snapshot_id for snapshot in (table.metadata.snapshots or [])} == expected_ids
    ...
    def test_side_effect_execution_boundary(self, seeded_table, duckdb_runner):
        ...
        with duckdb_runner() as test:
            ...
            test.query(
                ...
                SELECT
                    list_count(regexp_extract_all(message, '"action":"remove-snapshots"')),
                    list_count(regexp_extract_all(message, '"snapshot-ids"'))
                FROM commit_log
                """,
                [(1, 1)],
            )
            ...
            test.query(
                ...
                SELECT
                    ...
                    CAST(
                        ...
                        AND NOT contains(message, '"min-snapshots-to-keep"')
                        AND NOT contains(message, '"max-snapshot-age-ms"')
                        AND NOT contains(message, '"max-ref-age-ms"')
                        AND NOT contains(message, 'history.expire.')
                        AND NOT contains(message, 'gc.enabled')
                        AND NOT contains(message, 'metadata-version')
                        AND NOT contains(message, '"refs"')
                        AND NOT contains(message, 'assert-properties')
                        AS INTEGER
                    )
                FROM commit_log
                """,
