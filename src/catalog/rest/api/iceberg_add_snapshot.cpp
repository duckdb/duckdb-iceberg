...
//! MergeAppend: physically repack the assembled manifest list to reduce the number of manifest
//! files (#790). DATA and DELETE manifests are merged independently and never mixed. A manifest is
//! "new in this transaction" iff it was added by the snapshot we are building (`added_snapshot_id
...
void IcebergAddSnapshot::ApplyTableChanges(DatabaseInstance &db, ClientContext &context,
	...
	//! Use the operation the caller declared when staging this change (design doc B14): INSERT =>
	...
	for (auto &manifest_list_entry : uncommitted_manifest_files) {
		...
		//! would leak it on a failed/aborted commit (design doc B1). Over-tracking is safe: cleanup
	...
	//! MergeAppend (#790): physically reorganize the manifest list to bound manifest growth. This is a
