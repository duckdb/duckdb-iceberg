...
string Snapshot::TryFromJSON(yyjson_val *obj) {
	...
	if (parent_snapshot_id_val && !yyjson_is_null(parent_snapshot_id)) {
	...
	if (sequence_number_val && !yyjson_is_null(sequence_number)) {
	...
	if (added_rows_val && !yyjson_is_null(added_rows)) {
		has_added_rows = true;
	...
	if (schema_id_val && !yyjson_is_null(schema_id)) {
		has_schema_id = true;
